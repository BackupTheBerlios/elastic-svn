...
void copy_text(void)
{
	...
	if (!lflag)
		if (ec_flag)
			fprintf(f, ec_line_format, lineno, input_file_name);
		else
...
void copy_union(void)
{
	...
	{
		if (ec_flag)
			fprintf(text_file, ec_line_format, lineno, input_file_name);
		else
...
char *get_tag(void)
{
	...
	{
		tagmax += 16;
		tag_table = (char **) (tag_table ? REALLOC(tag_table, tagmax * sizeof(char *)) : MALLOC(tagmax * sizeof(char *)));
		if (tag_table == 0)
			no_space();
	}
	...
	if (c == '$')
	{
		if (cptr[1] == '<')
		{
			...
			} else if (isdigit(c))
			{
				...
				if (ec_flag)
					fprintf(f, "[self val_peek %d].%s", n - i, tag);
				else
			...
			} else if (c == '-' && isdigit(cptr[1]))
			{
				...
				i = -get_number() - n;
				if (ec_flag)
					fprintf(f, "[self val_peek %d].%s", -i, tag);
				else
...
int mark_symbol(void)
{
	...
	if (c == '=')
		cptr += 2;
			 ...
			 ((c = cptr[2]) == 'r' || c == 'R') &&
			 ((c = cptr[3]) == 'e' || c == 'E') && ((c = cptr[4]) == 'c' || c == 'C') && ((c = cptr[5], !IS_IDENT(c))))
		cptr += 5;
	else
		syntax_error(lineno, line, cptr);
